...
public final class DBOperations {
    ...
    public Cursor getSources() {
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.SOURCE);
        return database.rawQuery(sql, null);
    }
    public Cursor getSourcesById(String idSource) {
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s WHERE %s=?",
                OrderDB.Tables.SOURCE,
                OrderContract.Sources.ID);
        String[] whereArgs ={idSource};
        return database.rawQuery(sql, whereArgs);
    }

    public String insertSources(Source source) {
        SQLiteDatabase database = db.getWritableDatabase();
        String idSource = OrderContract.Sources.generateIdSource();
        ContentValues values = new ContentValues();
        values.put(OrderContract.Sources.ID, idSource);
        values.put(OrderContract.Sources.NAMESOURCE, source.getNameSource());
        values.put(OrderContract.Sources.TYPESOURCE, source.getTypeSource());

        return database.insertOrThrow(OrderDB.Tables.SOURCE,
                null, values) > 0 ? idSource : null;
    }

    public boolean updateSources(Source source) {
        SQLiteDatabase database = db.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(OrderContract.Sources.NAMESOURCE,
                source.getNameSource());
        values.put(OrderContract.Sources.TYPESOURCE,
                source.getTypeSource());
        String whereClause = String.format("%s=?",
                OrderContract.Sources.ID);
        final String[] whereArgs = {source.getIdSource()};
        int result = database.update(OrderDB.Tables.SOURCE,
                values, whereClause, whereArgs);
        return result > 0;
    }
    public boolean deleteSources(String idSources) {
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = String.format("%s=?",
                OrderContract.Sources.ID);
        final String[] whereArgs = {idSources};
        int result = database.delete(OrderDB.Tables.SOURCE, whereClause, whereArgs);
        return result > 0;
    }
